...
class Mapper(object):
    ...
    def __init__(self, cfg, args, slam):
        ...
        out_dir = os.path.join(self.output, "logs/mapping")
        os.makedirs(out_dir, exist_ok=True)
        self.txt = TextLogger(os.path.join(out_dir, "log.txt"))
    ...
    def optimize_map(
        self, rays_o, rays_d, rays_color, rays_depth, optimizer, num_joint_iters
    ):
        """
        ...
        for joint_iter in range(1, num_joint_iters + 1):
            ...
            with torch.enable_grad():
                ret = self.renderer.render_batch_ray(
                    rays_o=rays_o,
                    rays_d=rays_d,
                    net=self.mapping_net.to(device),
            ...
            uncertainty_weight = 1.0 / torch.sqrt(
                depth_variance.detach() + 1e-10
            )  # [n_rays, 1]
            if not self.uncertainty_based:
                uncertainty_weight = torch.ones_like(uncertainty_weight)

            assert (
                rays_depth.shape == est_depth.shape
            ), f"{rays_depth.shape}, {est_depth.shape}!"
            ...
            if self.w_sdf_loss > 0:
                sdf_loss, sparse_loss = self.mapping_net.compute_sdf_error(
                    sdf=sdf, z_vals=z_vals, gt_depth=rays_depth
                )
            ...
            if (self.local_step % self.num_joint_iters == 0) and self.verbose:
                ...
                msg += (
                    f" | Loss of total: {total_loss.detach():.4f}, depth: {depth_loss:.4f}, "
    ...
    def __call__(self, the_end=False):
        ...
        if cur_idx > 1:
            ...
            if self.last_visit > 0:
                ...
                visit_list += random_select(
                    self.last_visit, self.mapping_window_size - 12
                )
            ...
            for frame_id, frame in enumerate(visit_list):
                frame_items = self.video.get_mapping_item(
                    frame, device, decay=self.decay
                )
            ...
            for frame in unvisit_list:
                unvisit_frame[frame] = self.video.get_mapping_item(
                    frame, device, decay=self.decay
                )
            ...
            if (
                enable_ba and len(optimizer.param_groups) > 2
            ):  # Attention the number 2 set here
                del optimizer.param_groups[-1]
            if enable_ba and len(visit_ba_list) > 0:
                optimizer.add_param_group(
                    {"params": visit_ba_list, "lr": self.BA_cam_lr}
                )
            ...
            if self.verbose:
                self.txt.info(
                    prefix
                    + "Mapping Frame {}, unvisit {}, has visited {}".format(
                        timestamp.item(), unvisit_list, visit_list
                    )
                )
            else:
                if len(unvisit_list) > 2:
                    self.txt.info(
                        prefix
                        + "Mapping Frame {}, unvisit kf are: {}!".format(
                            timestamp.item(), unvisit_list
                        )
                    )
            ...
            if len(unvisit_list) > 2:
                ...
                for _ in range(unvisit_factor):
                    ...
                    sub_unvisit_list = list(
                        np.random.choice(unvisit_list, self.mapping_window_size)
                    )
