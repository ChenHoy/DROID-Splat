...
def is_positive_definite(matrix: torch.Tensor, eps=2e-5) -> bool:
    ...
    """
    return bool(
        (abs(matrix - matrix.mT) < eps).all()
        and (torch.linalg.eigvals(matrix).real >= 0).all()
    )
...
def block_solve(
    H: torch.Tensor, b: torch.Tensor, ep: float = 0.1, lm: float = 1e-4
) -> torch.Tensor:
...
def schur_block_solve(
    A: torch.Tensor, B: torch.Tensor, ep: float = 1.0, lm: float = 1e-4
) -> torch.Tensor:
    """block matrix solve used during Schur complement optimization."""
    b, n1, m1, p1, q1 = A.shape
    ...
    X = rearrange(X, "b (n1 p1) (m2 q2) -> b n1 m2 p1 q2", n1=n1, m2=m2, p1=p1, q2=q2)
    return X
...
def schur_solve(
    ...
    if structure_only:
        ...
        dX = schur_block_solve(S, y, ep=ep, lm=1e-4)
