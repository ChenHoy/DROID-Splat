...
def BA(target, weight, eta, poses, disps, intrinsics, ii, jj, fixedp=1, rig=1):
    ...
    vi = torch.matmul(wJiT, r).squeeze(dim=-1)
    vj = torch.matmul(wJjT, r).squeeze(dim=-1)

    Ei = (wJiT.view(B, N, D, ht * wd, -1) * Jz[:, :, None]).sum(dim=-1, keepdim=False)
    Ej = (wJjT.view(B, N, D, ht * wd, -1) * Jz[:, :, None]).sum(dim=-1, keepdim=False)

    w = w.view(B, N, ht * wd, -1)
    r = r.view(B, N, ht * wd, -1)
    wk = torch.sum(w * r * Jz, dim=-1, keepdim=False)
    Ck = torch.sum(w * Jz * Jz, dim=-1, keepdim=False)

    kx, kk = torch.unique(ii, return_inverse=True)
    M = kx.shape[0]

    # only optimize keyframe poses, rig: 1->mono, 2->stereo
    P = P // rig - fixedp
    ii = ii // rig - fixedp
    jj = jj // rig - fixedp
    ...
    E = safe_scatter_add_mat(Ei, ii, kk, P, M) + safe_scatter_add_mat(Ej, jj, kk, P, M)
...
# From original DROID-SLAM (somehow this got deleted here)
