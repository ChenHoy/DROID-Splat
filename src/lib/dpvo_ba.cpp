...
std::vector<torch::Tensor> solve_system(torch::Tensor J_Ginv_i, torch::Tensor J_Ginv_j, torch::Tensor ii, torch::Tensor jj, torch::Tensor res, float ep, float lm, int freen)
{
  ...
  // TODO chen: is this an artifact from testing in the original?
...
std::vector<torch::Tensor> solve_system_fixed(torch::Tensor J_Ginv_i, torch::Tensor J_Ginv_j, torch::Tensor ii, torch::Tensor jj, torch::Tensor res, float ep, float lm, int freen, int fixed_start = -1, int fixed_end = -1)
{
  const torch::Device device = res.device();
  J_Ginv_i = J_Ginv_i.to(torch::kCPU);
  J_Ginv_j = J_Ginv_j.to(torch::kCPU);
  ...
  // Add constraints for fixed poses
  if (fixed_start >= 0 && fixed_end >= fixed_start) {
    const double large_weight = 1e9;  // Very large weight to effectively fix variables
    
    // Apply large weights to diagonal blocks for fixed poses
    for (int idx = fixed_start; idx <= fixed_end && idx < n; idx++) {
      for (int d = 0; d < 7; d++) {
        // Use A.coeffRef to safely modify existing entries
        A.coeffRef(idx*7 + d, idx*7 + d) += large_weight;
        // Zero out the corresponding entry in b to ensure zero update
        b(idx*7 + d) = 0.0;
      }
    }
  }
  ...
  // Explicitly zero out updates for fixed poses for safety
  if (fixed_start >= 0 && fixed_end >= fixed_start) {
    for (int idx = fixed_start; idx <= fixed_end && idx < n; idx++) {
      for (int d = 0; d < 7; d++) {
        delta(idx*7 + d) = 0.0f;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("solve_system", &solve_system, "Solve system of equations for PGO");
  m.def("solve_system_fixed", &solve_system_fixed, "Solve system of equations for PGO but with fixed poses inside [t0, t1]");
  m.def("neighbors", &neighbors, "temporal neighboor indicies");
  m.def("reproject", &reproject, "Reproject patches");
}