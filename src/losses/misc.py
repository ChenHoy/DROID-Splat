...
from ..utils.image_utils import gradient_map


def l1(pred: torch.Tensor, gt: torch.Tensor) -> torch.Tensor:
    return torch.abs(pred - gt)


def l2(pred: torch.Tensor, gt: torch.Tensor) -> torch.Tensor:
    return (pred - gt) ** 2


def log_l1(pred: torch.Tensor, gt: torch.Tensor) -> torch.Tensor:
    return torch.log(1 + torch.abs(pred - gt))


# FIXME this is not the true masked mean!
def masked_loss(
    loss_func: callable,
    pred: torch.Tensor,
    gt: torch.Tensor,
    ...
    if loss_func.__name__ == "l1_huber_loss" or loss_func.__name__ == "pearson_loss":
        raise Exception("Please use loss {} directly!".format(loss_func.__name__))

    diff = loss_func(pred, gt)
...
def ms_masked_loss(
    ...
    for scale, w_l in zip(scales, lvl_weights):
        if scale == 1:
            # Original resolution
            total_loss += w_l * masked_loss(loss_func, pred, gt, weights=weights, mask=mask)
        else:
            ...
            total_loss += w_l * masked_loss(
                loss_func, scaled_output, scaled_gt, weights=scaled_weights, mask=scaled_mask
            )
    return total_loss


def l1_loss(pred: torch.Tensor, gt: torch.Tensor, **kwargs) -> float:
    return masked_loss(l1, pred, gt, **kwargs)


def l2_loss(pred: torch.Tensor, gt: torch.Tensor, **kwargs):
    return masked_loss(l2, pred, gt, **kwargs)


def log_l1_loss(pred: torch.Tensor, gt: torch.Tensor, **kwargs) -> float:
    return masked_loss(log_l1, pred, gt, **kwargs)
