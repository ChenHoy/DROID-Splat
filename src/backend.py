...
class Backend:
    ...
    def ba(
        ...
        rawd = deepcopy(d).reshape(ilen, jlen)  # Distance matrix for loop closure
        ...
        n_neighboring = 1  # TODO why is this always 1?
        while len(ix) > 0:
            k = ix.pop(0)
            di, dj = k // jlen, k % jlen
            ...
            if loop:
                ...
                for si in range(
                    ...
                    for sj in range(
                        ...
                        # Loop detection with threshold for motion between the keyframes
                        if rawd[(si - t_start_loop), (sj - t_start)] <= thresh:
                            num_loop += 1
                            if si != sj:
                                sub_es += [(si, sj)]
                # TODO why does the number of loops need to be greater than 9 / 2 = 4.5?
                if num_loop > int(((n_neighboring * 2 + 1) ** 2) * 0.5):
                    es += sub_es  # TODO why do we just add the to loopy edges that need to be substituted?
            else:
                es += [
                    (i, j),
                ]
        ...
        # We only run bundle adjustment if we have enough edges
        # NOTE this might be one of the reasons why this code here runs that much faster,
        # it does not always run the local frontend when its not necessary
        # FIXME will this create similar results to vanilla DROID-SLAM?
        # if len(es) < 3:
        #     return 0
