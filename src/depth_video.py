...
class DepthVideo:
    def __init__(self, cfg, args):
        ...
        if args.camera_model == "pinhole":
            ...
            raise Exception(
                "Camera model not implemented! Choose either pinhole or mei model."
            )
        ...
        ).share_memory_()  # w2c quaterion
        self.poses_gt = torch.zeros(
            buffer, 4, 4, device=device, dtype=torch.float
        ).share_memory_()  # c2w matrix
        ...
        self.fmaps = torch.zeros(
            buffer, c, 128, ht // s, wd // s, dtype=torch.half, device=device
        ).share_memory_()
        self.nets = torch.zeros(
            buffer, 128, ht // s, wd // s, dtype=torch.half, device=device
        ).share_memory_()
        ...
        self.poses_filtered = torch.zeros(
            buffer, 7, device=device, dtype=torch.float
        ).share_memory_()  # w2c quaterion
        ...
        self.poses_filtered[:] = torch.tensor(
            [0, 0, 0, 0, 0, 0, 1], dtype=torch.float, device=device
        )
        ...
        self.update_priority = torch.zeros(
            buffer, device=device, dtype=torch.float
        ).share_memory_()
        self.bound = torch.zeros(
            1, 3, 2, device=device, dtype=torch.float
        ).share_memory_()

        ### pose compensation from vitural to real
        self.pose_compensate = torch.zeros(
            1, 7, dtype=torch.float, device=device
        ).share_memory_()
        self.pose_compensate[:] = torch.tensor(
            [0, 0, 0, 0, 0, 0, 1], dtype=torch.float, device=device
        )
    ...
    def get_mapping_item(self, index, device="cuda:0", decay=0.1):
        with self.mapping.get_lock():
            image = (
                self.images[index].clone().permute(1, 2, 0).contiguous().to(device)
            )  # [h, w, 3]
            ...
            # origin alignment
            w2c = lietorch.SE3(self.poses_filtered[index].clone()).to(
                device
            )  # Tw(droid)_to_c
            c2w = (
                lietorch.SE3(self.pose_compensate[0].clone()).to(w2c.device) * w2c.inv()
            )
            c2w = c2w.matrix()  # [4, 4]
