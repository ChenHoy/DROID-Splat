...
def color_loss(
    ...
    if with_ssim:
        if ms_ssim:
            ssim_loss_func = MS_SSIM(data_range=1.0, size_average=False)
        else:
            ssim_loss_func = SSIM(data_range=1.0, size_average=True)
        ssim_loss = ssim_loss_func(
...
class SSIM(torch.nn.Module):
    ...
    def _ssim(self, X: Tensor, Y: Tensor, mask: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
        """Calculate ssim index for X and Y

        Args:
            X, Y (torch.Tensor): images
        ...
        win = self.win.to(X.device, dtype=X.dtype)
    ...
    def ssim(self, X: Tensor, Y: Tensor, mask: Optional[Tensor] = None) -> Tensor:
        r"""interface of ssim
        Args:
            X, Y  (torch.Tensor): a batch of images, (N,C,H,W)
            mask (torch.Tensor): boolean mask same size as X and Y
        ...
        ssim_per_channel, cs = self._ssim(X, Y, mask=mask)
    ...
    def forward(self, X: Tensor, Y: Tensor, mask: Optional[Tensor] = None) -> Tensor:
        return self.ssim(X, Y, mask=mask)


class MS_SSIM(SSIM):
    def __init__(self, weights: Optional[List[float]] = None, *args, **kwargs) -> None:
        r"""class for ms-ssim
        Args:
            data_range (float or int, optional): value range of input images. (usually 1.0 or 255)
            size_average (bool, optional): if size_average=True, ssim of all images will be averaged as a scalar
            ...
            K (list or tuple, optional): scalar constants (K1, K2). Try a larger K2 constant (e.g. 0.4) if you get a negative or NaN results.
        """
        super(MS_SSIM, self).__init__(*args, **kwargs)
        self.weights = weights

    def ms_ssim(self, X: Tensor, Y: Tensor, mask: Optional[Tensor] = None) -> Tensor:
        r"""interface of ms-ssim
        Args:
            X, Y  (torch.Tensor): a batch of images, (N,C,[T,]H,W)
            mask (torch.Tensor): boolean mask same size as X and Y
        Returns:
        ...
        assert smaller_side > (self.win_size - 1) * (
            2**4
