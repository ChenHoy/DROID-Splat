...
def build_rays(
    ...
    if dir_normalize:
        ...
        raise TypeError(
            "Ray direction shouldn't be normalized, otherwise the scale of pose will be destroyed!"
        )
...
def build_all_rays(
    H, W, fx, fy, cx, cy, c2w, device, nerf_coordinate=True, dir_normalize=False
):
    """
...
def sample_pdf(bins, weights, N_importance, det=False):
    ...
    if det:
        u = torch.linspace(
            0.0 + 0.5 / N_importance, 1 - 0.5 / N_importance, steps=N_importance
        )
    ...
    cdf_g = torch.gather(
        cdf.unsqueeze(dim=1).expand(matched_shape), dim=2, index=inds_g
    )
    bins_g = torch.gather(
        bins.unsqueeze(dim=1).expand(matched_shape), dim=2, index=inds_g
    )
